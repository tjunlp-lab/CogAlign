...
def model_input(batch,m,i,temp_order,train_word,train_label,train_seqlen,train_eye_feature,train_eeg_feature,is_text,task_label):
    temp_word = []
    temp_label = []
    temp_length = []
    temp_eye_feature = []
...
def main(_):
    ...
    for fold in range(fold_num):
        ...
        train_eye_feature = np.load(data_path+'/fold_{}/train_eye_feature.npy'.format(fold))
        train_eeg_feature = np.load(data_path+'/fold_{}/train_eeg_feature.npy'.format(fold))
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():  
                ...
                with tf.variable_scope('sentiment_model',reuse=tf.AUTO_REUSE,initializer=initializer):
                    ...
                    if mode == 'single':
                        for i in range(len(temp_order)//setting.batch_size):
                            feed_dict = model_input(setting.batch_size,m,i,temp_order,train_word,
                                            train_label,train_seqlen,train_eye_feature,train_eeg_feature,1,task_text)                               
                        ...
                        for i in range(len(temp_order)//setting.batch_size):
                            for j in range(2):
                                if j==0:
                                    ...
                                    feed_dict = model_input(setting.batch_size,m,i,temp_order,train_word,
                                                   train_label,train_seqlen,train_eye_feature,train_eeg_feature,0,task_cognitive) 
