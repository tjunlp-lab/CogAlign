...
def main(_):
    ...
    for fold in range(fold_num):
        ...
        test_pos_feature = np.load(data_path+'/fold_{}/test_pos_feture.npy'.format(fold))
        test_relative_positions = np.load(data_path+'/fold_{}/test_relative_positions.npy'.format(fold))
        test_reverse_relative_positions = np.load(data_path+'/fold_{}/test_reverse_relative_positions.npy'.format(fold))
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                with tf.variable_scope('relation_model'):
                ...
                for k in range(1000,6000,500):   
                    ...
                    feed_dict[m.pos_input] = np.asarray(test_pos_feature)
                    feed_dict[m.relative_position_input] = np.asarray(test_relative_positions)
                    feed_dict[m.reverse_position_input] = np.asarray(test_reverse_relative_positions)
